...
<section class="bg-base-200 py-10" id="code">
  <div class="container mx-auto px-4">
    <div class="text-center">
      <h2 class="text-3xl font-bold text-base-content mb-6">Pinned Repositories</h2>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      {:else}
        {#each repos as { link, owner, repo, description, languageColor, language, stars, forks }}
          <a href={link} target="_blank" rel="noreferrer" class="card bg-secondary text-secondary-content shadow-xl p-6 transition-transform transform hover:scale-105">
            <div class="flex justify-between items-center mb-4">
              <div class="flex items-center">
                <img src={`https://github.com/${owner}.png`} alt="{owner}'s profile picture" class="w-8 h-8 rounded-full mr-2" />
            ...
            <div class="flex justify-between items-center text-sm">
              {#if language}
                <div class="flex items-center">
                  <span class="dot w-3 h-3 rounded-full" style="background-color: {languageColor}"></span>
              ...
              {#if forks}
                <div class="flex items-center">
                  <GitForkIcon size="16px" class="mr-1 text-secondary-content" />
                  <span>{forks}</span>
                </div>
              {/if}
            </div>
